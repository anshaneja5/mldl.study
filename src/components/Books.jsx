...
const books = [
  ...
  { title: 'Deep Learning for Coders with fastai and PyTorch', author: 'Jeremy Howard, Sylvain Gugger', link: 'https://course.fast.ai/Resources/book.html' },
...
const Books = () => {
  ...
  return (
    <>
      ...
      {/* Main Books Component Section */}
      <div className={`min-h-screen ${darkMode ? 'bg-black' : 'bg-white'} ${darkMode ? 'text-white' : 'text-black'} p-4 md:p-8`}>
            ...
            {articles.map((article, index) => (
              <li key={index} className="text-lg">
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className={`underline ${darkMode ? 'text-green-300' : 'text-green-600'}`}
                >
                  {article.title}
                </a>{' '}
                by {article.author}
